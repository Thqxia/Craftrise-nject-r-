...
DWORD GetGamePID() {
	HWND hwGame = FindWindowA(0, "CraftRise"); //Pencere Adý Exe Adý Deðil!
	if (!hwGame) return 0;
	DWORD ret = 0;
...
int main()
{
	...
	if (!gamePID) {
		MessageBoxA(0, "Oyun Bulunamadý", "Lütfen Oyunu Baþlatýn!", 0);
		return 0;
	}
	HookBypass::SetGamepid(gamePID);
	std::string dllpath = SelectDll();
	if (dllpath == "") {
		MessageBoxA(0, "Dll Dosyasý Seçilmedi", "Lütfen Bir Dll Dosyasý Seçin!", 0);
		return 0;
	}
	...
	MessageBoxA(0, "Enjekte Baþarýlý!", "CrInjectorc++", 0);
	return 0;
}
